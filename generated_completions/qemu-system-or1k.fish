...
complete -c qemu-system-or1k -o smp -d 'Simulate a SMP system with \'n\' CPUs initially present on the machine type boa…'
...
complete -c qemu-system-or1k -o global -d 'Set default value of driver\'s property prop to value, e. g. : NDENT 7'
...
complete -c qemu-system-or1k -o audio -d 'If the model option is specified, -audio is a shortcut for configuring both t…'
...
complete -c qemu-system-or1k -o qmp -d 'Like -monitor but opens in \'control\' mode'
...
complete -c qemu-system-or1k -s S -d 'Do not start CPU at startup (you must type \'c\' in the monitor)'
...
complete -c qemu-system-or1k -s d -d 'Enable logging of specified items.  Use \'-d help\' for a list of log items'
...
complete -c qemu-system-or1k -o no-shutdown -d 'Don\'t exit QEMU on guest shutdown, but instead only stop the emulation'
complete -c qemu-system-or1k -o action -d 'The action parameter serves to modify QEMU\'s default behavior when certain gu…'
...
complete -c qemu-system-or1k -o nodefaults -d 'Don\'t create default devices'
...
complete -c qemu-system-or1k -o semihosting -d 'Enable \\%Semihosting mode (ARM, M68K, Xtensa, MIPS, RISC-V only)'
complete -c qemu-system-or1k -o semihosting-config -d 'Enable and configure \\%Semihosting (ARM, M68K, Xtensa, MIPS, RISC-V only)'
